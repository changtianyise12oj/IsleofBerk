package com.GACMD.isleofberk.common.entity.entities.base.path;
        ...
        for(int i = pX; i < pX + pXSize; ++i) {
            for(int j = pY; j < pY + pYSize; ++j) {
                for(int k = pZ; k < pZ + pZSize; ++k) {
                    ...
                    if (fluidstate.isEmpty() && blockstate.isPathfindable(pBlockaccess, blockpos$mutableblockpos.below(), PathComputationType.AIR) && blockstate.isAir()) {
