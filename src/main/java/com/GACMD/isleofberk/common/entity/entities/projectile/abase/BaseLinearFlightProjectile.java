...
//         kill entity when ticks exceed 250 to remove lag
    ...
    public void shootNoScaling(Vec3 end, float partialTicks, float pInaccuracy) {
        ...
        // plays particles
        // use rotation and endpoint
        Vec3 vec3 = (endVec).normalize().add(this.random.nextGaussian() * 0.007499999832361937D * (double) pInaccuracy, this.random.nextGaussian() * 0.007499999832361937D * (double) pInaccuracy, this.random.nextGaussian() * 0.007499999832361937D * (double) pInaccuracy);
        this.setDeltaMovement(vec3);
