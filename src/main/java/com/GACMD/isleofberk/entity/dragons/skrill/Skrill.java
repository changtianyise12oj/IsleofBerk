...
import static com.GACMD.isleofberk.registery.ModTags.Items.SKRILL_TAME_FOOD;
import static com.GACMD.isleofberk.registery.ModTags.Items.SKRILL_BREED_FOOD;

public class Skrill extends ADragonBaseFlyingRideableProjUser {
    ...
    public Skrill(EntityType<? extends Skrill> entityType, Level level) {
    ...
    public AgeableMob getBreedOffspring(@NotNull ServerLevel level, @NotNull AgeableMob parent) {
        Skrill dragon = ModEntities.SKRILL.get().create(level);
        return dragon;
    }
    ...
    protected void playerFireProjectile(Vec3 riderLook, Vec3 throat) {
        if ((tier1() || tier2() || tier3() || tier4()) && !isUsingAbility()) {
            ...
            SkrillLightning bolt = new SkrillLightning(this, throat, riderLook, level, getExplosionStrength());
    ...
    public void dragonShootProjectile(Vec3 dragonLook, Vec3 throat) {
        if ((tier1() || tier2() || tier3() || tier4()) && !isUsingAbility()) {
            setTicksSinceLastFire(ticksSinceLastProjShootSet());
            SkrillLightning bolt = new SkrillLightning(this, throat, dragonLook, level, getExplosionStrength());
